...
## Theme C — Stakes and Consequence

### C2. Failure consequences on traits [BUILD]

When a Goal fails or a high-pressure Want is repeatedly frustrated, the associated trait (by tag match) takes a weight penalty and gains a `setback_note`. Repeated failures against the same trait eventually trigger a belief: "I don't seem to be the kind of person who [trait name]" — which suppresses re-emergence of that trait for a period.

Depends on: trait system (done).

---

### C5. Attachment patterns [BUILD]

Part of the person model. Over time Chloe develops a characteristic attachment style per person — derived from warmth history, outreach attempts, response rates, and conflict events. Generated by Haiku from relationship history, stored as text in `persons`.

Influences: default risk tolerance, which traits activate in conversation, whether distance decays normally or is resisted.
