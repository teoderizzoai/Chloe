...
Kept frozen at starting values (`EI=58, SN=62, TF=44, JP=67`) for `heart.py` compatibility only. `tick_vitals` and `auto_decide` still use `soul_activity_affinity()` with MBTI floats. No longer drifts. Do not extend.
...
5. Auto-regulate activity
   override = auto_decide(vitals, activity, hour, mood, soul)  # soul still used here for affinity
